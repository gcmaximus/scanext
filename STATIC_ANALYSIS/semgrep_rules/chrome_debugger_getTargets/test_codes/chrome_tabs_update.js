...
function kms(n, a) {
    ...
    chrome.tabs.update(a, { abc: "", url: n[0].url, cdb: "sdf" }, (tab) => {
        console.log(tab);
    }); // Expect 1 match

    let rite12 = { abc: "", url: n[0].title, cdb: "sdf" }; // Expect 1 match
    chrome.tabs.update(rite12);

    let rite13 = { abc: "", url: n[0].url, cdb: "sdf" }; // Expect 1 match
    chrome.tabs.update(rite13, (tab) => {
        console.log(tab);
    });

    chrome.tabs.update({ abc: "", url: n[0].title, cdb: "sdf" }); // Expect 1 match

    console.log();

    let x = n[0].faviconUrl;
    ...
    x = n[0];
    chrome.tabs.update(a, { abc: "", url: x.url, cdb: "sdf" }, (tab) => {
        console.log(tab);
    }); // Expect 1 match here
...
function aaa(n, a) {
    ...
    let rite22 = { abc: "", url: n[0].title, cdb: "sdf" }; // Expect 1 match
    chrome.tabs.update(rite22);

    let rite23 = { abc: "", url: n[0].url, cdb: "sdf" }; // Expect 1 match
...
chrome.debugger.getTargets(function (n, a) {
    ...
    let rite32 = { abc: "", url: n[0].title, cdb: "sdf" }; // Expect 1 match
    chrome.tabs.update(rite32);

    let rite33 = { abc: "", url: n[0].url, cdb: "sdf" }; // Expect 1 match
...
chrome.debugger.getTargets(function bbb(n, a) {
    ...
    let rite42 = { abc: "", url: n[0].title, cdb: "sdf" }; // Expect 1 match
    chrome.tabs.update(rite42);

    let rite43 = { abc: "", url: n[0].url, cdb: "sdf" }; // Expect 1 match
