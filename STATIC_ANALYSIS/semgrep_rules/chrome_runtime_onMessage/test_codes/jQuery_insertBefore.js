...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  // Insert the content insertBefore the first element matching the selector
  $('body').find('selector').first().insertBefore(content);//1 match here
});
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  // Insert the image insertBefore the element matching the selector
  $(selector).insertBefore(image); // 1 match here
});
