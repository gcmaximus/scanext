...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  // Insert the content before the first element matching the selector
  $('body').find('selector').first().before(content);//1 match here
});
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  // Insert the image before the element matching the selector
  $(selector).before(image); // 1 match here
});
