...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  // Insert the content after the first element matching the selector
  $('body').find('selector').first().after(content);//1 match here
});
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  // Insert the image after the element matching the selector
  $(selector).after(image); // 1 match here
});
